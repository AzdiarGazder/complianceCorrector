...
% This script demonstrates how to automatically correct uniaxial tension
...
% Figures comprising:
% - Plot: Compliance corrected force vs displacement
...
%% *** REQUIRED USER INPUT ***
alloyElements = 'Zr, Ti, Nb';
% Alloy composition is assumed to be in weight percent or weight fraction
% Use the flag 'atomic' in the calcModulus command if the alloy
% composition is in atomic percent or atomic fraction
alloyComposition = [35, 40, 25];
% alloyComposition = [59, 19, 22];
%% *** 
...
gaugeLength_mm = []; % gauge length (in mm)
...
[E,~] = calcModulus('elements',alloyElements,...
...
%% Save the uncorrected displacement and force data corresponding to the
% entire test duration
d_mm_unCorrected = d_mm;
f_kN_unCorrected = f_kN;
%%
...
d_mm = [d_mm_elastic; d_mm_plastic];
...
d_mm_corrected = d_mm - (f_kN_corrected ./ k_loadFrame);
...
figure;
plot(d_mm_unCorrected, f_kN_unCorrected,'.-k');
hold all;
...
engStress = (f_kN_corrected.*10^3)./csArea_mm2;
figure;
