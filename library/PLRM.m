function [coeffs, crossOverPts, crossOverIdx, R2] = PLRM(xData, yData, n)
...
%                 intersect
%  crossOverIdx - @double, a column vector of the closest index of the x
%                 and y coordinate indicating where adjacent segments
%                 intersect
...
%% Find the index of each crossover point in the original xData and yData
% For each crossover x value, find the index of the nearest xData value.
if ~isempty(crossOverPts)
    crossOverIdx = zeros(size(crossOverPts, 1), 1);
    for s = 1:size(crossOverPts, 1)
        [~, idx] = min(abs(xData - crossOverPts(s, 1)));
        crossOverIdx(s) = idx;
    end
else
    crossOverIdx = [];
